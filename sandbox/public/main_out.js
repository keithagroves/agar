(function (f, r) {
    ...
    function ia() {
        ...
        r.get("http://m.agar.io/info", function (a) {
            ...
            for (c in b) r('#region option[value="' + c + '"]')
                .text(S[c] + " (" + b[c] + " players)")
        }, "json")
    }
    ...
    function Da(a) {
        ...
        for (;;) {
            ...
            c += 4;
            if (0 == d) break;
            for (var e = a.getFloat32(c, !0), c = c + 4, k = a.getFloat32(c, !0), c = c + 4, f = a.getFloat32(c, !0), c = c + 4, h = a.getUint8(c++), l = a.getUint8(c++), p = a.getUint8(c++), h = (h << 16 | l << 8 | p)
            ...
            n.nSize = f;
            n.updateCode = b;
            n.updateTime = E; - 1 != B.indexOf(d) && -1 == g.indexOf(n) && (document.getElementById("overlays")
                .style.display = "none", g.push(n), 1 == g.length && (s = n.x, t = n.y))
        }
        a.getUint16(c, !0);
        c += 2;
        k = a.getUint32(c, !0);
        ...
        if (null != u || 0 != y.length)
            if (null != u || Z) {
                ...
                a.fillText(c, 100 - a.measureText(c)
                    ...
                    2, 40);
                if (null == u)
                    for (a.font = "20px Ubuntu", b = 0; b < y.length; ++b) c = y[b].name || "An unnamed cell", Z || (c = "An unnamed cell"), -1 != B.indexOf(y[b].id) ? (g[0].name && (c = g[0].name), a.fillStyle = "#FFAAAA") : a.fillStyle = "#FFFFFF", c = b + 1 + ". " + c, a.fillText(c, 100 - a.measureText(c)
                        .width / 2, 70 + 24 * b);
                else
